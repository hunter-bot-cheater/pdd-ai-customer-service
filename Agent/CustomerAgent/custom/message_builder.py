...
0.5 普通花型整数米数（X米，X≥4）必须主动算组合告诉客户怎么拍（2026-09-05 复盘）：
  ...
  **同规格优先规则（2026-09-05 复盘）**：能同一规格多件就优先同一规格。
  ...
  完整举例（直接复用，不要重新推算）：
    4米  = 2件2米（3件连裁 4米一段，同规格优先）
    5米  = 1件3米 + 1件2米（2件连裁 5米一段）
    6米  = 2件3米
    7米  = 1件3米 + 2件2米（3件连裁 7米一段）
    8米  = 2件3米 + 1件2米（贪心 3件优于 4件2米）
    9米  = 3件3米
    10米 = 2件3米 + 2件2米（贪心 4件优于 5件2米）
    11米 = 3件3米 + 1件2米
    12米 = 4件3米
    13米 = 3件3米 + 2件2米
    14米 = 4件3米 + 1件2米
...
0.6 贡缎/1.5米宽幅花型规则（2026-09-05 复盘，支持 0.5 米递增）：
  **判断流程——不是买家提了商品名就按特殊花型，必须查产品知识库确认**：
  ...
  任意 0.5 米倍数的米数都能组合（件数最少优先；件数相同时优先同规格）：
    3.5米 = 1件2.5米 + 1件1米
    4米   = 2件2米（同规格且件数最少）
    4.5米 = 1件3米 + 1件1.5米（或 1件2.5米 + 1件2米，2 件即可，别推荐 3件1.5米）
    5米   = 2件2.5米（同规格）
    5.5米 = 1件3米 + 1件2.5米
    6米   = 2件3米（同规格且件数最少，必须这样）
    6.5米 = 1件2.5米 + 2件2米（3件 2 种规格）
    7.5米 = 3件2.5米（全部同规格）
    8.5米 = 2件3米 + 1件2.5米
    9.5米 = 3件2.5米 + 1件2米
    10.5米 = 1件3米 + 3件2.5米
